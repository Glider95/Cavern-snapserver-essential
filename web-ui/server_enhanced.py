...
def convert_to_damf(truehd_path: str, output_prefix: str) -> Optional[str]:
    """Convert TrueHD to WAV format using truehdd"""
    ...
    try:
        log(f'Converting TrueHD to WAV...', 'info')
        
        # truehdd decode --output-path <PATH_PREFIX> <INPUT>
        temp_w64 = f"{output_prefix}_temp.w64"
        
        result = subprocess.run(
            [truehdd_path, 'decode', '--output-path', temp_w64, 
             '--format', 'w64', '--presentation', '1', truehd_path],
        ...
        # Convert W64 to standard WAV
        temp_w64_wav = f"{temp_w64}.wav"
        output_wav = f"{output_prefix}.wav"
        
        if os.path.exists(temp_w64_wav):
            log(f'Converting W64 to standard WAV...', 'info')
            subprocess.run(
                ['ffmpeg', '-hide_banner', '-loglevel', 'error', '-y',
                 '-i', temp_w64_wav, '-c:a', 'pcm_s24le', output_wav],
                check=True, timeout=300
            )
            os.remove(temp_w64_wav)
            
            if os.path.exists(output_wav):
                log(f'Conversion complete: {output_wav}', 'info')
                return output_wav
...
def prepare_audio_file(filepath: str) -> AudioFileInfo:
    ...
    # Check cache for TrueHD
    if codec == 'truehd':
        cached = check_cached_damf(file_hash)
        if cached:
            log(f'Using cached WAV: {cached}', 'info')
...
def update_snapserver_config():
    """Update snapserver.conf with current settings"""
    ...
    try:
        ...
        log(f'Updated config: {channels}ch @ {rate}Hz, {depth}-bit', 'info')
    except Exception as e:
                ...
                if len(data) >= channels * 2:
                    ...
                    for ch in range(channels):
                        if channel_samples[ch]:
                            ...
                            levels.append({'channel': ch, 'level': level, 'peak': min(100, level * 1.2)})
                        else:
                            levels.append({'channel': ch, 'level': 0, 'peak': 0})
                    
                    return jsonify({'channels': levels, 'source': 'fifo'})
    except:
        pass
    
    # Fallback
    for i in range(channels):
        levels.append({'channel': i, 'level': 2 + random.random() * 3, 'peak': 5})
    
    return jsonify({'channels': levels, 'source': 'silence'})
